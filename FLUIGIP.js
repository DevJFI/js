...
function FLUIGIP(){
	...
	this.updCalendar = function(){
		...
		$(document).on('keydown', ".calendar", function(e) {
			var validos = '1234567890/Backspace'
			if(validos.indexOf(e.key) < 0)
				return false;
		});
	}
	/**
	...
	*/
	...
	this.USEFUL = {
		...
		validFloatValue: function(value, allowZero){
			
			var valid = false;
			
			// is a number
			if(typeof allowZero == 'undefined' || eval(allowZero) == "false")
			//if(allowZero == false || allowZero =)
	...
	this.VALID = {
		...
		validateForm: function(){
			var obj = this;
			var isValidate = true;
			var messageAll = "Preencha os campos obrigatórios antes de avançar.";
			var messageVld = "";
			var result = [];
			...
			$("input,select,textarea").each(function(){
				...
				if ($(this).attr("type") == "zoom"){
					...
					allowZero	= $(this).data("fluig-allow-zero");
				}
				...
				if ( (typeof required != 'undefined') && (typeof states != 'undefined') && (validConditional) ){
					...
					if ( eval(required) && (states.indexOf(getWKNumState()) != -1  || states.indexOf("*") != -1) ) {
						...
						if ( value.trim() == "" || value =='undefined' || value == null || FLUIGIP.USEFUL.validFloatValue(value, allowZero) ){
