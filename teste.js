...
function afterTaskSave23(colleagueId,nextSequenceId,userList){
	var idgrupo="TI";
	var usuario = getValue('WKUser');
	...
	if(nextSequenceId =="5"){
		email_abertura()
		cria_task(nextSequenceId);
		IdTags();
	}
	
	if(tipo!="Linha" && tipo!="Celular"){
		/*	
			ETAPA 17 - ORÇAMENTO: OCORRE NO ARQUIVO TI_ReqEquipamentoV2.servicetask17.js
		{
			CRIA TICKET ASANA NA TAREFA DA DIGITAL
			
		}
			 ETAPA 47 - AGUARDANDO ORÇAMENTO: OCORRE NO ARQUIVO TI_ReqEquipamentoV2.servicetask47.js
		{
			A cada 1 hora executa script onde verifica se há algum anexo na task do Asana na tarefa da Digital (subtarefa)
		...
		// ETAPA 77 - RETORNAR ORÇAMENTO: OCORRE NO ARQUIVO TI_ReqEquipamentoV2.servicetask77.js
		

		if(nextSequenceId==87 || nextSequenceId==89 || nextSequenceId ==20 || nextSequenceId==22 || nextSequenceId==107){
			atualiza_task(nextSequenceId)
			movimenta_task('1201786043276003', hAPI.getCardValue('IDASANA'))//JFI
			movimenta_task('1203701610894702', hAPI.getCardValue("IDASANAFILHO"))//DIGITAL
			var aprovadorpor = hAPI.getCardValue('aprovadores')
			if(nextSequenceId=='87' || nextSequenceId=='107'){//Gerente Regional || Gestor CDC
		...
		// ETAPA 28 - APROVADO: OCORRE NO ARQUIVO TI_ReqEquipamentoV2.servicetask22.js
		if(nextSequenceId==66 || nextSequenceId== 25 || nextSequenceId==94 || nextSequenceId==110){//REPROVADO
			if( hAPI.getCardValue('IDASANA_Estoque')!=''){
				movimenta_task('1208408547857428', hAPI.getCardValue('IDASANA_Estoque'))//
				var ST_Estoque = hAPI.getCardValue('ST_Estoque')
				var texto=ST_Estoque+' - Notebook'
		...
		/*if(nextSequenceId=='20' || nextSequenceId=='22'){//ANALISE GESTOR || ANALISE DIRETORIA
			movimenta_task('1203701382703719', hAPI.getCardValue('IDASANA'))//JFI TELEFONIA
			if(nextSequenceId=='20'){
				NomeAprovador(hAPI.getCardValue('NOME_APROVADOR'))
			}
			else{
				NomeAprovador('FELIPE VALVASSORI DE ALMEIDA')
			}
		}*/
	...
	if(nextSequenceId =='25' || nextSequenceId =='66'){//REPROVADO
		email_reprova()
